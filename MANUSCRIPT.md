...
Application of the Theil-Sen slope estimator to the gap-filled, continuous TWS record (April 2002–December 2019) reveals widespread negative TWS trends across the 103 basins analyzed (Figure 4). Exactly 42 out of 103 basins (40.8%) exhibit statistically significant ($p < 0.05$, Hamed and Rao Modified Mann-Kendall test) negative trends, with a mean decline rate of -0.528 cm/year among these declining basins. The most severe depletions are observed in the Kura-Araks Basin (Basin 36, -2.847 km³/year, $p < 10^{-15}$), the Ganges-Brahmaputra Basin (Basin 51, -20.694 km³/year, $p = 2.68 \times 10^{-8}$), the Fraser Basin (Basin 14, -2.857 km³/year, $p = 1.57 \times 10^{-12}$), the Yukon Basin (Basin 5, -9.724 km³/year, $p = 1.33 \times 10^{-15}$), and the Don Basin (Basin 17, -4.073 km³/year, $p = 2.82 \times 10^{-5}$). Conversely, 17 basins (16.5%) show significant positive TWS trends, primarily located in high-latitude and humid tropical zones, reflecting increased precipitation regimes. The remaining 44 basins (42.7%) exhibit no statistically significant secular trend.
...
> *Description:* SHAP Summary Plot for the Ganges-Brahmaputra (Basin 51). A scatter plot where the y-axis represents the 5 driver variables, the x-axis represents the SHAP value (impact on TWSC prediction), and color indicates the feature value magnitude (low to high). Shows strong negative SHAP values for high Groundwater Abstraction. Generated by `plot_shap_summary.m`.  
> **Figure 12.** SHAP (SHapley Additive exPlanations) Summary Plot for the Ganges-Brahmaputra Basin (Basin 51), demonstrating the directional impact of hydroclimate and abstraction drivers on the Random Forest $TWSC$ prediction. Each point represents a single month in the testing period. High groundwater abstraction volumes (red) consistently yield negative SHAP values, independently pulling the predicted water storage trajectory downward.

> **[INSERT Figure 13 HERE]**  
> *Description:* Bar chart comparing mean prediction bias in Pristine vs. Irrigated basins from the Spatial Transferability Test. Irrigated basins exhibit a strong positive bias (over-prediction) because the $M_{nat}$ model lacks the anthropogenic depletion sink. Generated by `plot_spatial_transferability.m`.  
...
At the level of the machine learning framework itself, the Random Forest approach, while powerful in capturing nonlinear relationships, is not a physically constrained model and may conflate correlated drivers. The OOB permutation importance metric assumes feature independence, an assumption violated when predictors are correlated (e.g., $P$ and $Q$). Future work should explore conditional permutation importance (Strobl et al., 2008) or SHAP (SHapley Additive exPlanations) values to provide more robust and interpretable feature attribution. Finally, the 213-month analysis period (2002–2019), while representing the longest available satellite gravimetry record, may be insufficient to fully characterise decadal-scale climate variability modes, potentially aliasing multi-decadal signals as secular trends.
...
The following supplementary materials accompany this manuscript. Table S1 provides a complete basin-by-basin summary including Basin ID, TWS trend (cm/year), Modified Mann-Kendall p-value, significance flag, Block CV NSE ($M_{nat}$ and $M_{anthro}$), Block CV KGE ($M_{nat}$ and $M_{anthro}$), OOB $R^2$ ($M_{nat}$ and $M_{anthro}$), $\Delta R^2$, and dominant driver category for all 103 basins. Figure S1 shows individual basin time series of observed and reconstructed TWS anomalies for all 103 basins in a multi-page panel figure. Figure S2 presents a global map of OOB $R^2$ for the gap-filling Random Forest model, showing spatial variability in reconstruction quality. Figure S3 provides the correlation matrix of predictor variables ($P, ET, Q, GW_{abs}, SW_{abs}$) across representative basins, illustrating inter-predictor dependencies. All MATLAB code used in this study is available at `[repository URL]`. Processed basin-level time series and attribution results are archived at `[DOI/repository]`.
